...
(() => {
  ...
  function createBoard() {
    /** @type {readonly ["left" | "right" | "top" | "bottom", number][][]} */
  ...
  /**
   * @param {HTMLButtonElement} figureElm
  ...
  function canPlayerMove(diceValue = 0) {
    ...
    const startPlayerPositionElm = /** @type {HTMLDivElement} */ (
    ...
    const canPlayerMoveOnPath = Array.from(fieldElms).some((fieldElm) => {
      ...
      if (playerFigureElm?.parentElement) {
        const playerFigurePositionIndex = Array.from(fieldElms).indexOf(
          /** @type {HTMLDivElement} */ (playerFigureElm.parentElement)
        );
  ...
  function handleFigureClick(figureElm, i) {
    ...
    if (isFigureInHouse) {
      ...
      if (diceValue === 6) {
        const startPositionElm = /** @type {HTMLDivElement} */ (
          document.querySelectorAll(".start-pos")[i]
        );
      ...
      const figurePositionIndex = figureElm.parentElement
        ? Array.from(fieldElms).indexOf(
            /** @type {HTMLDivElement} */ (figureElm.parentElement)
          )
        : 0;
  ...
  /**
   * @param {HTMLButtonElement} diceElm
   * @param {number} i
   */
